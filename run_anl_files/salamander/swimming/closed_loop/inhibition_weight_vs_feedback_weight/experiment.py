...
INHIBITION_GAIN_ARRAY_TYPES = {
    'wide' : np.linspace(0, 6.0, 31),
    'zoom' : np.linspace(0, 1.0, 31),
}

def run_experiment(
    ps_gain_axial           : float,
    inhibition_gain_type    : str,
    ...
    inhibition_gain_array = INHIBITION_GAIN_ARRAY_TYPES[inhibition_gain_type]
    simulation_data_file_tag = (
        'inhibition_weight_vs_feedback_weight_'
        f'{inhibition_gain_type}_'
    ...
    params_runs = [
        ...
        for inhibition_gain in inhibition_gain_array
    ...
    for params_processes_batch in params_processes_batches:
        simulate_multi_net_multi_run_closed_loop(
            modname             = f'{CURRENTDIR}/net_farms_limbless_inhibition_weight_ps_weight.py',
