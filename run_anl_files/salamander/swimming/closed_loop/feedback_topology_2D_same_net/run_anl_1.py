...
def main():
    ...
    # Parameters
    simulation_data_file_tag = 'feedback_topology_strength_swimming_2dim'
    ...
    params_runs = (
        [
            ...
            for sig_ex_up in np.linspace(0.0, max_ps_range, 11)
            for sig_ex_dw in np.linspace(0.0, max_ps_range, 11)
        ] +
        [
            {
                'sig_ex_up' : 0,
            ...
            for sig_in_up in np.linspace(0.0, max_ps_range, 11)
            for sig_in_dw in np.linspace(0.0, max_ps_range, 11)
        ]
    )
