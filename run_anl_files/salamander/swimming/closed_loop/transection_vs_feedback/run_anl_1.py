...
def main():
    ...
    # Parameters
    simulation_data_file_tag = 'transection_cpg_ps_vs_feedback'
    ...
    params_runs = [
        {
            'ps_gain_axial'         : default.get_scaled_ps_gains(ps_gain_scaling),
            'n_transections'        : n_transections,
            'transected_populations': ['cpg', 'ps'],
        }
        for n_transections in np.arange(21)
        for ps_gain_scaling in np.linspace(0.05, 1, 20)
    ]
    ...
    for params_processes_batch in params_processes_batches:
        simulate_multi_net_multi_run_closed_loop(
            modname             = f'{CURRENTDIR}/net_farms_limbless_transection.py',
