...
def run_experiment(
    ...
    simulation_data_file_tag = (
        'swimming_cpg_neural_parameters_'
        f'alpha_{round(alpha_fraction_th * 100) :03d}_'
        f'ps_weight_{round(ps_weight_nominal * 100) :03d}'
    )

    ps_weight_range   = (
        [0.00, 0.001]
        if ps_weight_nominal == 0.0
        else
        [ ps_weight_nominal - 0.05, ps_weight_nominal + 0.05]
    )
